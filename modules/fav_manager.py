...
class UserFavModal(disnake.ui.Modal):
    ...
    async def callback(self, inter: disnake.ModalInteraction):
        ...
        await inter.edit_original_message(
            embed=disnake.Embed(
                description="**Lưu/cập nhật liên kết thành công trong yêu thích của bạn!\n"
                            "Nó sẽ xuất hiện trong khi:** ```\n"
                            "- Khi sử dụng lệnh /play (trong tìm kiếm tự động tìm kiếm)\n"
                            "- Khi nhấp vào nút chạm yêu thích của người chơi.\n"
                            "- Khi sử dụng lệnh chơi (tiền tố) mà không có tên hoặc liên kết.```",
...
class UserFavView(disnake.ui.View):
    ...
    def __init__(self, bot: BotCore, ctx: Union[disnake.AppCmdInter, CustomContext], data: dict):
        ...
        if data["fav_links"]:
            play_button = disnake.ui.Button(label="Chơi yêu thích được chọn ", emoji="▶")
    ...
    async def remove_callback(self, inter: disnake.MessageInteraction):
        ...
        await inter.edit_original_message(
            embed=disnake.Embed(
                description="**Liên kết/Yêu thích đã được gỡ bỏ thành công!**",
...
class FavManager(commands.Cog):
    ...
    async def fav_manager(self, inter: disnake.AppCmdInter):
        ...
        if user_data["fav_links"]:
            ...
            if cog:
                ...
                embed.add_field(name="**Lệnh dùng như thế nào?**", inline=False,
                                value=f"* Sử dụng lệnh {cmd} (Trong tìm kiếm tự động tìm kiếm)\n"
                                      "* Bằng cách nhấp vào.\n"
                                      f"* lệnh {global_data['prefix'] or self.bot.default_prefix}{cog.play_legacy.name}mà không bao gồm tên hoặc liên kết cho một bài hát/video hoặc .\n"
                                      "* Sử dụng nút yêu thích bên dưới.")
    ...
    async def modal_import(self, inter: disnake.ModalInteraction):
        ...
        for name in json_data.keys():
            if len(name) > (max_name_chars := self.bot.config["USER_FAV_MAX_NAME_LENGTH"]):
                await inter.edit_original_message(
                    f"**Một mục từ tệp của bạn ({name}) vượt quá số lượng ký tự được phép:{max_name_chars}**")
                return
            try:
        ...
        if self.bot.config["MAX_USER_FAVS"] > 0 and not (await self.bot.is_owner(inter.author)):
            ...
            if (json_size + (user_favs := len(user_data["fav_links"]))) > self.bot.config["MAX_USER_FAVS"]:
                await inter.edit_original_message(
                    "Bạn không có đủ không gian để thêm tất cả các mục yêu thích của tệp của mình..\n"
        ...
        await inter.edit_original_message(
            embed=disnake.Embed(
                ...
                description="**Các liên kết đã được nhập thành công!**\n"
                            f"**Sử dụng lệnh {cmd} Để kiểm tra nó (trong tìm kiếm tự động tìm kiếm).**",
            )
        )
    ...
    async def export_(self, inter: disnake.MessageInteraction):
        ...
        if retry_after:
            if retry_after < 1:
                retry_after = 1
            await inter.send("**Bạn phải đợi {} xuất khẩu.**".format(
                time_format(int(retry_after) * 1000, use_names=True)), ephemeral=True)
            return
        ...
        embed = disnake.Embed(
            description=f"Yêu thích của bạn ở đây. \ Bạn có thể nhập bằng lệnh: {cmd}",
            color=self.bot.get_color())
