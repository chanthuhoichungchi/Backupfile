...
class GuildFavModal(disnake.ui.Modal):
    ...
    async def callback(self, inter: disnake.ModalInteraction):
        ...
        await inter.edit_original_message(embed=disnake.Embed(description="**Liên kết được thêm/cập nhật thành công vào ghim của người chơi!\n"
                          "Thành viên có thể sử dụng trực tiếp trong player-controller khi không sử dụng.**",
    ...
    async def remove_callback(self, inter: disnake.MessageInteraction):
        ...
        try:
            ...
            await inter.edit_original_message(
                embed=disnake.Embed(
                    description=f"**Không có liên kết trong danh sách với tên:** {self.current}",
...
class PinManager(commands.Cog):
    ...
    async def server_playlist(self, inter: disnake.AppCmdInter):
        ...
        embed = disnake.Embed(
            description="**Trình quản lý yêu thích máy chủ.**",
    ...
    async def modal_import(self, inter: disnake.ModalInteraction):
        ...
        for name in json_data.keys():
            if len(name) > (max_name_chars := 25):
                await inter.edit_original_message(f"**Một mục từ tệp của bạn ({name}) vượt quá số lượng ký tự cho phép:{max_name_chars}**")
                return
            try:
        ...
        if (json_size:=len(json_data)) > 25:
            await inter.edit_original_message(f"Số lượng mục trong tệp vượt quá số lượng tối đa được phép (25).")
            return

        if (json_size + (user_favs:=len(guild_data["player_controller"]["fav_links"]))) > 25:
            await inter.edit_original_message("Danh sách/danh sách phát nhạc của máy chủ không có đủ không gian để thêm tất cả các mục trong Archiv của bạno...\n"
    ...
    async def export_(self, inter: disnake.MessageInteraction):
        ...
        if not guild_data["player_controller"]["fav_links"]:
            await inter.edit_original_message(content=f"**Không có bài hát/danh sách phát cố định trên máy chủ..\n"
        ...
        embed = disnake.Embed(
            description=f"**Dữ liệu từ danh sách nhạc/nhạc cố định của máy chủ đang ở đây.\n"
