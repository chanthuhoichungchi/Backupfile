    ...
    async def callback(self, inter: disnake.ModalInteraction):
        ...
        if (matches := spotify_regex_w_user.match(url)):
            ...
            if not self.bot.spotify:
                await inter.send(
                    embed=disnake.Embed(
                        description="**Hỗ trợ Spotify không có sẵn tại thời điểmo...**",
                        color=disnake.Color.red()
                    ), ephemeral=True
                )
                return
            ...
            if not result:
                await inter.send(
                    embed=disnake.Embed(
                        description="**Người dùng liên kết thông tin không có danh sách phát công cộng...**",
                        color=self.bot.get_color()
                    )
                )
                return
            ...
            if not self.bot.config["USE_YTDL"]:
                await inter.send(
                    embed=disnake.Embed(
                        description="**Không có hỗ trợ cho loại liên kết này vào lúc này...**",
                        color=self.bot.get_color()
                    )
                )
                return
            ...
            try:
                ...
                traceback.print_exc()
                await inter.edit_original_message(f"**Xảy ra lỗi khi lấy thông tin từ URL:** ```py\n{repr(e)}```")
                return

            if not info:

                msg = f"**Người dùng/kênh của liên kết được thông báo không tồn tại:**\n{url}"

                if source == "[YT]:":
                    msg += f"\n\n`Lưu ý: Kiểm tra xem liên kết có chứa người dùng với @, ví dụ: @ytchannel`"
            ...
            if not info['entries']:
                await inter.edit_original_message(
                    embed=disnake.Embed(
                        description=f"**Người dùng/kênh của liên kết thông tin không có danh sách phát công khai...**",
                        color=disnake.Color.red()
                    )
                )
                return
        ...
        await inter.edit_original_message(
            embed=disnake.Embed(
                description=f"**Tích hợp thêm/chỉnh sửa thành công:** [`{title}`]({data['url']})\n"
                            "**Nó sẽ xuất hiện vào những dịp sau đây:** ```\n"
                            "- Khi sử dụng lệnh /play (trong tìm kiếm tự động tìm kiếm)\n"
                            "-Khi nhấp vào nút yêu thích của người chơir.\n"
                            "-Khi sử dụng lệnh chơi (tiền tố) không có tên hoặc liên kết.```",
...
class IntegrationsView(disnake.ui.View):
    ...
    def __init__(self, bot: BotCore, ctx: Union[disnake.AppCmdInter, CustomContext], data: dict):
        ...
        if data["integration_links"]:
            play_button = disnake.ui.Button(label="Chơi vào tích hợp đã chọn", emoji="▶")
    ...
    async def interaction_check(self, inter: disnake.MessageInteraction) -> bool:
        ...
        await inter.send(f"Chỉ có thành viên {self.ctx.author.mention} mới có thể tương tác trong tin nhắn này.", ephemeral=True)
...
class IntegrationManager(commands.Cog):
    ...
    @commands.command(name="integrations", aliases=["integrationmanager", "itg", "itgmgr", "itglist", "integrationlist"],
                      description="Quản lý tích hợp của nó.", cooldown=itg_cd)
    ...
    async def integrations(self, inter: disnake.AppCmdInter):
        ...
        if user_data["integration_links"]:
            ...
            if cog:
                ...
                embed.add_field(name="**Làm thế nào để phát danh sách phát của một tích hợp?**", inline=False,
                                value=f"* Sử dụng lệnh {cmd} (Trong tìm kiếm tự động tìm kiếm)\n"
                                      "* Bằng cách nhấp vào.\n"
                                      f"* Sử dụng lệnh {global_data['prefix'] or self.bot.default_prefix}{cog.play_legacy.name} mà không bao gồm tên hoặc liên kết cho một bài hát/video.\n"
                                      "* Sử dụng nút cảm ứng bên dưới.")
    ...
    async def modal_import(self, inter: disnake.ModalInteraction):
        ...
        try:
            ...
            await inter.send("**Đã xảy ra lỗi khi phân tích dữ liệu hoặc dữ liệu không hợp lệ/không định dạngo "
        ...
        if self.bot.config["MAX_USER_INTEGRATIONS"] > 0 and not (await self.bot.is_owner(inter.author)):
            ...
            if (json_size := len(json_data)) > self.bot.config["MAX_USER_INTEGRATIONS"]:
                await inter.edit_original_message(f"Số lượng mục trong tệp tích hợp của bạn vượt quá "
                                   f"Số lượng tối đa cho phép ({self.bot.config['MAX_USER_INTEGRATIONS']}).")
                return
    ...
    async def export_(self, inter: disnake.MessageInteraction):
        ...
        embed = disnake.Embed(
            description=f"Tích hợp của nó là ở đây.\nBạn có thể nhập bằng lệnh: {cmd}",
            color=self.bot.get_color())
