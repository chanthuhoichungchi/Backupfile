...
class Owner(commands.Cog):
    ...
    @commands.is_owner()
    @panel_command(aliases=["rd", "recarregar"], description="Tải lại các mô-đun.", emoji="🔄",
                   alt_name="Tải/Nạp lại mô-đun.")
    async def reload(self, ctx: Union[CustomContext, disnake.MessageInteraction]):
        ...
        if data["loaded"]:
            txt += f'**Tải lên các Module <:ll:1138141608924172339>** ```ansi\n[0;34m{" [0;37m| [0;34m".join(data["loaded"])}```\n'

        if data["reloaded"]:
            txt += f'**Tải lại các Module <:ll:1138141608924172339>** ```ansi\n[0;32m{" [0;37m| [0;32m".join(data["reloaded"])}```\n'

        if not txt:
            txt = "**<:ll:1138141608924172339> Đéo có Module nào đc tải cả hihihi...**"
    ...
    async def update(self, ctx: Union[CustomContext, disnake.MessageInteraction], *,
        ...
        if not os.path.isdir("./.git") or args.force:
            ...
            try:
                ...
                if "Already up to date" in pull_log:
                    raise GenericError("** <:ll:1138141608924172339> Tôi đã cài đặt các bản cập nhật cuối cùng ...**")
                out_git += pull_log
        ...
        text = "`Bạn cần phải khởi động lại sau khi cập nhật.`"

        txt = f"`✅` **[Cập nhật thực hiện thành công!]({self.bot.pool.remote_git_url}/commits/main)**"
    ...
    async def update_deps(self, ctx, original_reqs, args, use_poetry=False):
        ...
        if args.pip:
            ...
            embed = disnake.Embed(
                description="**Cài đặt các phụ thuộc.\nVui lòng chờ...**",
                color=self.bot.get_color(ctx.guild.me)
            )
                ...
                await ctx.send(
                    embed=disnake.Embed(
                        description="**Nó sẽ là cần thiết để cập nhật các phụ thuộc bằng cách sử dụng lệnh "
                                    ...
                                    f"và 0,5VCPU) Bạn phải gửi tệp yêu cầu.txt thay vì "
    ...
    async def panel(self, ctx: CustomContext):
        ...
        embed =disnake.Embed(
            title="<:ll:1138141608924172339> BẢNG ĐIỀU KHIỂN.",
            color=self.bot.get_color(ctx.guild.me)
        )
    ...
    @commands.command(description="Đồng bộ hóa/ghi các lệnh thanh trên máy chủ.", hidden=True)
