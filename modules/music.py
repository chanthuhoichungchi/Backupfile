...
class Music(commands.Cog):
    ...
    async def addcache(self, ctx: CustomContext, url: str):
        ...
        await ctx.send("Các bài hát liên kết đã được thêm thành công vào bộ đệm.", delete_after=30)
    ...
    async def updatecache(self, ctx: CustomContext, *args):
        ...
        try:
            if not self.bot.pool.playlist_cache:
                raise GenericError("**Bộ đệm danh sách phát của bạn trống ...**")
        except KeyError:
        ...
        for url in list(self.bot.pool.playlist_cache):
            ...
            embed = disnake.Embed(
                ...
                title=f"Playlist đã xác minh: {counter}/{amount}"
            )
    ...
    async def removecache(self, ctx: CustomContext, url: str):
        ...
        try:
            ...
            raise GenericError("**Không có mục được lưu trong bộ đệm với URL thông báo...**")
    ...
    async def clearcache(self, ctx: CustomContext):
        ...
        try:
            ...
            raise GenericError("**Bạn không lưu các liên kết danh sách phát trong bộ đệm...**")
    ...
    async def importcache(self, ctx: CustomContext, url: str):
        ...
        await ctx.send("Tệp bộ đệm đã được nhập thành công! ", delete_after=30)
    ...
    @pool_command(
        ...
        description="Kích hoạt hệ thống thông báo tự động sân khấu với tên của bài hát.",
    ...
    @commands.slash_command(
        description=f"{desc_prefix}Kích hoạt/chỉnh sửa hệ thống thông báo tự động giai đoạn với tên của âm nhạc.",
    ...
    async def stage_announce(
            ...
            template: str = commands.Param(
                ...
                description=f"{desc_prefix}Kích hoạt hệ thống thông báo tự động giai đoạn với tên của âm nhạc."
            )
    ):
        ...
        if not isinstance(guild.me.voice.channel, disnake.StageChannel):
            raise GenericError("**Bạn phải ở trên một kênh giai đoạn để kích hoạt/vô hiệu hóa hệ thống này.**")
        ...
        if not template:
            ...
            raise GenericError(
                ...
                "[34;1m{track.author}[0m -> Tên của nghệ sĩ/tải lên/tác giả của bài hát.\n"
                "[34;1m{track.duration}[0m -> Thời lượng âm nhạc.\n"
                "[34;1m{track.source}[0m -> Nguồn gốc/nguồn âm nhạc (Youtube/Spotify/Soundcloud etc)\n"
                ...
                "[34;1m{requester.id}[0m -> ID của thành viên đã yêu cầu bài hát\n```"
            )

        if player.stage_title_event and player.stage_title_template == template:

            raise GenericError("**Thông báo tự động của sân khấu đã được kích hoạt (và không có thay đổi nào trong "
                               "Mẫu tiêu đề).\n"
                               "Nếu bạn muốn vô hiệu hóa, bạn có thể dừng người chơi (tất cả các thành viên sân khấu sẽ"
        ...
        txt = [f"đã kích hoạt/thay đổi hệ thống thông báo sân khấu.",
               f"📢 **⠂{inter.author.mention} đã kích hoạt/thay đổi hệ thống thông báo sân khấu "
               f"{guild.me.voice.channel.mention}.**\n\n"
               f"`Lưu ý: Nếu người chơi bị tắt, tất cả các thành viên trên sân khấu sẽ tự động bị ngắt kết nối.`\n\n"
               f"**Mô hình được sử dụng:** `{disnake.utils.escape_markdown(template)}`"]
    ...
    async def stage_announce_autocomplete(self, inter: disnake.Interaction, query: str):
        ...
        return [
            ...
            "Đài phát thanh 24/7 | {track.title}",
            "{track.title} | Danh sách phát: {track.playlist}",
            "Đang nghe: {track.title}",
            "Chill with me {track.title} | {track.source}",
            "Chill cùng {requester.name}, đang phát {track.title}",
        ]
    ...
    @pool_command(name="addposition", description="Thêm âm nhạc vào một dòng cụ thể trong hàng đợi.",
                  aliases=["adp", "addpos"], check_player=False, cooldown=play_cd, max_concurrency=play_mc,
                  usage="{prefix}{cmd} [Vị trí(Nº)] [nome|link]\nEx: {prefix}{cmd} 2 sekai - burn me down")
    async def addpos_legacy(self, ctx: CustomContext, position: int, *, query: str):

        if position < 1:
            raise GenericError("**Số vị trí đường dây phải là 1 hoặc cao hơn.**")
    ...
    play_flags.add_argument('-position', '-pos', '-p', type=int, default=0, help='Đặt âm nhạc ở một vị trí cụ thể.\nEx: -p 10')
    play_flags.add_argument('-next', '-proximo', action='store_true', help='Thêm nhạc/danh sách phát ở đầu dòng (tương đương với: -pos 1)')
    play_flags.add_argument('-reverse', '-r', action='store_true', help='Đảo ngược thứ tự của các bài hát được thêm (chỉ có hiệu lực khi thêm danh sách phát).')
    play_flags.add_argument('-shuffle', '-sl', action='store_true', help='Trộn thêm các bài hát (chỉ hiệu quả khi thêm danh sách phát).')
    play_flags.add_argument('-select', '-s', action='store_true', help='Chọn âm nhạc từ kết quả được tìm thấy.')
    play_flags.add_argument('-source', '-scr', type=str, default="ytsearch", help='Tìm kiếm âm nhạc bằng một nguồn cụ thể [youtube/soundcloud etc]')
    play_flags.add_argument('-force', '-now', '-n', '-f', action='store_true', help='Phát nhạc được thêm ngay lập tức (chỉ có hiệu lực nếu có một bài hát ngày hôm nay.)')
    play_flags.add_argument('-loop', '-lp', type=int, default=0, help="Xác định số lượng lặp lại của âm nhạc đã chọn.\nEx: -loop 5")
    play_flags.add_argument('-server', '-sv', type=str, default=None, help='Sử dụng một máy chủ âm nhạc cụ thể.')

    @can_send_message_check()
    @check_voice()
    @commands.bot_has_guild_permissions(send_messages=True)
    @commands.max_concurrency(1, commands.BucketType.member)
    @pool_command(name="play", description="Chơi nhạc trên kênh thoại.", aliases=["p"], check_player=False,
    ...
    @commands.slash_command(
        ...
        description=f"{desc_prefix}Phát tệp nhạc trên kênh thoại.",
    ...
    async def play(
        ...
        if not query:
            ...
            if len(db_favs) == 1:
                ...
                if not select_interaction or view.selected is False:
                    ...
                    text = "### Thời gian lựa chọn đã hết!" if view.selected is not False else "### Bị người dùng hủy bỏ."
        ...
        if query.startswith(("> fav: ", "> itg: ")):
            ...
            if not urls:
                ...
                if not self.bot.config["ENABLE_DISCORD_URLS_PLAYBACK"] and "cdn.discordapp.com/attachments/" in query:
                    raise GenericError("**Hỗ trợ liên kết Discord bị vô hiệu hóa.**")
                ...
                if "&list=" in query and (link_re := YOUTUBE_VIDEO_REG.match(query)):
                    ...
                    view = SelectInteraction(
                        user=inter.author,
                        opts=[
                            disnake.SelectOption(label="Âm nhạc", emoji="🎵",
                                                 description="Chỉ tải nhạc từ liên kết.", value="music"),
                            disnake.SelectOption(label="Playlist", emoji="🎶",
                                                 description="Chơi danh sách phát với bài hát hiện tại.", value="playlist"),
                        ], timeout=30)
                    ...
                    try:
                        if bot.user.id != self.bot.user.id:
                            embed.set_footer(text=f"Thông qua: {bot.user.display_name}",
                    ...
                    if not view.inter or view.selected is False:
                        ...
                        await func(
                            content=f"{mention}{'Đã hủy hoạt động' if view.selected is not False else 'Đã hết thời gian'}" if view.selected is not False else "Bị người dùng hủy bỏ.",
                ...
                if not bot_perms.read_message_history:
                    ...
                    if not bot_perms.manage_permissions:
                        ...
                        player.set_command_log(
                            emoji="⚠️",
                            text=f"Tôi không được phép xem tin nhắn trên kênh: {channel_check.mention} "
                                 f"(và không cho phép quản lý quyền để tự động sửa chữa điều này),"
        ...
        if isinstance(tracks, list):
            ...
            if manual_selection and len(tracks) > 1:
                ...
                embed.description = f"**Chọn bài hát mong muốn bên dưới:**"
            ...
            if not isinstance(tracks, list):
                ...
                if force_play == "yes":
                    ...
                    pos_txt = f" trong vị trí {position + 1} hàng ngang"
    ...
    @check_voice()
    @commands.slash_command(
        description=f"{desc_prefix}Bỏ qua đến một bài hát cụ thể trong dòng.",
    ...
    async def skipto(
            ...
            query: str = commands.Param(
                name="name",
                description="Tên bài hát (hoàn chỉnh hoặc một phần của nó)."
            ),
    ...
    async def skip(
            ...
            play_only: str = commands.Param(
                ...
                description="Chỉ cần chơi nhạc ngay lập tức (mà không xoay dòng)",
                default="no"
            ),
            case_sensitive: bool = commands.Param(
                name="exact_name", default=False,
    ...
    @commands.slash_command(
        ...
        description=f"{desc_prefix}Bỏ phiếu để nhảy âm nhạc hiện tại.",
        extras={"only_voiced": True}
    )
    ...
    async def volume_legacy(self, ctx: CustomContext, level: int):
        ...
        if not 4 < level < 151:
            raise GenericError("**Âm lượng không hợp lệ! Chọn từ 5 đến 150**", self_delete=7)
    ...
    async def volume(
            self,
            inter: disnake.AppCmdInter, *,
            value: int = commands.Param(name="level", description="cấp độ từ 5 đến 150", min_value=5.0, max_value=150.0)
    ):
    ...
    async def resume(self, inter: disnake.AppCmdInter):
        ...
        txt = ["tiếp tục âm nhạc.", f"▶️ **⠂{inter.author.mention} Tiếp tục bài hát**"]
        await self.interaction_message(inter, txt, rpc_update=True, emoji="▶️")
    ...
    @check_voice()
    @commands.slash_command(
        description=f"{desc_prefix}Chọn Chế độ lặp lại giữa: Dòng điện / Hàng đợi hoặc Tắt.",
    ...
    async def remove(
            self,
            inter: disnake.AppCmdInter,
            query: str = commands.Param(name="name", description="Hoàn thành tên âm nhạc."),
            case_sensitive: bool = commands.Param(
                name="exact_name", default=False,
                description="Tìm kiếm âm nhạc với lời bài hát chính xác thay vì tìm kiếm từng từ trong tên của âm nhạc",
        ...
        txt = [
            f"loại bỏ bài hát [`{(fix_characters(track.title, 25))}`]({track.uri or track.search_uri}) da fila.",
    ...
    @pool_command(name="readd", aliases=["readicionar", "rdd"], only_voiced=True, cooldown=queue_manipulation_cd,
                  max_concurrency=remove_mc, description="Đưi các bài hát được chơi vào lại trong hàng.")
    ...
    @check_voice()
    @commands.slash_command(
        description=f"{desc_prefix}Đưa các bài hát được chơi vào lại trong hàng.",
    ...
    move_args.add_argument('-casesensitive', '-cs',  action='store_true',
                           help="Tìm kiếm lời bài hát chính xác thay vì tìm kiếm từng từ trong tên "
                                "Từ bài hát")
    move_args.add_argument('-position', '-pos', help="Chỉ định vị trí đích (đây là cho các tình huống trong đó tên để tìm kiếm bắt đầu bằng số).\nEx: -pos 1", type=int, default=None)

    @is_dj()
    @has_player()
    @check_voice()
    @pool_command(name="move", aliases=["mv", "mover"], only_voiced=True, max_concurrency=remove_mc,
                  description="Di chuyển các bài hát đến một dòng cụ thể trong dòng.", extras={"flags": move_args},
                  usage="{prefix}{cmd} <posição(nº)> [nome]\nEx 1: {prefix}{cmd} 2 sekai\nEx 2: {prefix}{cmd} sekai\n"
                        "(Lưu ý: Nếu một vị trí không được chỉ định qua ARG hoặc FLAG, âm nhạc sẽ được chuyển đến vị trí 1)")
    async def move_legacy(self, ctx: CustomContext, position: Optional[int] = None, *, flags: str = ""):
        ...
        if args.position:
            ...
            raise GenericError("**Bạn đã không báo cáo một vị trí trong hàng.**")
    ...
    @check_voice()
    @commands.slash_command(
        description=f"{desc_prefix}Di chuyển một bài hát đến vị trí được chỉ định trong dòng.",
        extras={"only_voiced": True}, max_concurrency=remove_mc
    )
    async def move(
            self,
            inter: disnake.AppCmdInter,
            query: str = commands.Param(name="name", description="Tên của bài hát."),
            ...
            case_sensitive: bool = commands.Param(
                name="exact_name", default=False,
                description="Tìm kiếm âm nhạc theo thuật ngữ chính xác thay vì tìm kiếm từng từ trong tên của âm nhạc",
        ...
        if (i_size := len(indexes)) == 1:
            ...
            txt = [
                f"Chuyển sang âm nhạc [`{fix_characters(track.title, limit=25)}`]({track.uri or track.search_uri}) cho vị trí **[{position}]** trong hàng.",
                f"↪️ **⠂{inter.author.mention} đã di chuyển một bài hát cho vị trí [{position}]:**\n"
            ...
            if i_size > 20:
                embed.description += f"\n\n`Và hơn thế nữa {i_size - 20} Bài hát`"
            ...
            if ephemeral:
                player.set_command_log(
                    text=f"{inter.author.mention} di chuyển **[{i_size}]** Bài hát có tên **{fix_characters(query, 25)}"
                         f"** cho vị trí**[{position_text}]** trong hàng.", emoji="↪️")
    ...
    @pool_command(name="rotate", aliases=["rt", "rotacionar"], only_voiced=True,
                  description="Xoay dòng sang nhạc được chỉ định.",
    ...
    async def song_request_thread(self, inter: disnake.AppCmdInter):
        ...
        if not player.controller_mode:
            raise GenericError("**Vùng da/hiện tại không tương thích với hệ thống yêu cầu bài hát "
                               "thông qua chủ đề/hội thoạia\n\n"
    ...
    async def clear(
            self,
            inter: disnake.AppCmdInter,
            song_name: str = commands.Param(name="name_of_the_song", description="Bao gồm tên bạn có trong âm nhạc.",
                                            default=None),
            song_author: str = commands.Param(name="author_name",
                                              description="Bao gồm tên bạn có trong tác giả của bài hát.", default=None),
            user: disnake.Member = commands.Param(name='user',
                                                  description="Bao gồm âm nhạc được yêu cầu bởi người dùng đã chọn.",
                                                  default=None),
            duplicates: bool = commands.Param(name="duplicate", description="Bao gồm các bài hát trùng lặp",
                                              default=False),
            playlist: str = commands.Param(description="Bao gồm tên bạn có trên danh sách phát.", default=None),
            min_duration: str = commands.Param(name="initial_duration",
                                               description="Bao gồm các bài hát có thời lượng cuối cùng/bằng (Ex. 1:23).",
                                               default=None),
            max_duration: str = commands.Param(name="maximum_duration",
                                               description="Bao gồm các bài hát có thời lượng tối đa được chỉ định (ví dụ. 1:45).",
                                               default=None),
            range_start: int = commands.Param(name="start_position",
                                              description="Bao gồm các bài hát dòng từ một vị trí cụ thể "
                                                          "từ một hàng.",
                                              min_value=1.0, max_value=500.0, default=None),
            range_end: int = commands.Param(name="end_position",
                                            description="Bao gồm nhạc dòng vào một vị trí dòng cụ thể.",
                                            min_value=1.0, max_value=500.0, default=None),
            absent_members: bool = commands.Param(name="absent_members",
                                                  description="Bao gồm các bài hát được thêm vào của các thành viên ngoài kênh",
                                                  default=False)
    ):
    ...
    @clear.autocomplete("author_name")
    ...
    async def player_message_delete(self, message: disnake.Message):
        ...
        player.message = None
        await thread.edit(archived=True, locked=True, name=f"lưu trữ: {thread.name}")
        ...
        for guild_id in list(self.bot.music.players):
            ...
            try:
                ...
                if not player.is_paused and not player.is_playing:
                    await player.process_next()
                print(f"{self.bot.user} - {player.guild.name} [{guild_id}] - Trình phát được kết nối lại trên kênh thoại")
            except:
                traceback.print_exc()
                    ...
                    if isinstance(channel_db, disnake.Thread):
                        ...
                        if not channel_db.parent:
                            ...
                            if (channel_db.archived or channel_db.locked) and not channel_db.parent.permissions_for(
                                ...
                                raise GenericError(
                                    f"**{bot.user.mention} không có quyền quản lý chủ đề cho "
                                    f"hủy lưu trữ/mở khóa chủ đề: {channel_db.mention}**")
                ...
                if channel_db:
                    ...
                    if not channel_db_perms.send_messages:
                        raise GenericError(
                            ...
                            "Nếu bạn muốn đặt lại cài đặt của kênh, hãy sử dụng lại lệnh /reset hoặc /setup "
                            "..."
                        )
    ...
    async def guild_pin(self, interaction: disnake.MessageInteraction):
        ...
        if not self.bot.bot_ready:
            await interaction.send("Tôi vẫn đang khởi động...\nVui lòng đợi lâu hơn một chút...", ephemeral=True)
            return
        ...
        try:
            ...
            return await interaction.send("**Mục được chọn không được tìm thấy trong cơ sở dữ liệu...**", ephemeral=True)
    ...
    async def player_controller(self, interaction: disnake.MessageInteraction, control: str, **kwargs):
        ...
        if not self.bot.bot_ready:
            await interaction.send("Tôi vẫn đang khởi động...", ephemeral=True)
            return
        ...
        try:
            ...
            if control == PlayerControls.add_song:
                ...
                if not interaction.user.voice:
                    raise GenericError("**Bạn phải nhập kênh thoại để sử dụng nút này.**")
                ...
                await interaction.response.send_modal(
                    ...
                    components=[
                        disnake.ui.TextInput(
                            ...
                            placeholder="Tên hoặc liên kết youtube/spotify/soundcloud etc.",
            ...
            if control == PlayerControls.enqueue_fav:
                ...
                if player.interaction_cooldown:
                    raise GenericError("Người chơi đang ở trong thời gian hồi chiêu, thử lại trong khoảnh khắc.")
                ...
                if not interaction.author.voice or interaction.author.voice.channel != vc:
                    raise GenericError(f"Bạn phải ở trên kênh <#{vc.id}> sử dụng các nút của trình phát.")
                ...
                try:
                    ...
                    raise GenericError(
                        "**Bạn có một tương tác mở!**\n`Và đó là một tin nhắn ẩn, tránh nhấp vào \"phớt lờ\".`")
                ...
                if control == PlayerControls.add_favorite:
                    ...
                    if self.bot.config["MAX_USER_FAVS"] > 0 and not (await self.bot.is_owner(interaction.author)):
                        ...
                        if len(user_data["fav_links"]) >= self.bot.config["MAX_USER_FAVS"]:
                            await interaction.response.edit_message(
                                embed=disnake.Embed(
                                    color=self.bot.get_color(interaction.guild.me),
                                    description="Bạn không có đủ không gian để thêm tất cả các mục yêu thích từ tệp của mình...\n"
                                                "Giới hạn hiện tại: {self.bot.config['MAX_USER_FAVS']}"
                                ), view=None)
                            return
                    ...
                    await interaction.edit_original_response(
                        embed=disnake.Embed(
                            color=self.bot.get_color(interaction.guild.me),
                            description="### Bài hát được thêm/chỉnh sửa thành công vào mục yêu thích của bạn:\n\n"
    ...
    async def song_requests(self, ctx: Optional[CustomContext], message: disnake.Message):
            ...
            if not self.bot.intents.message_content:
                ...
                await message.channel.send(
                    ...
                    components=[
                        ...
                        disnake.ui.Button(emoji="⭐", custom_id=PlayerControls.enqueue_fav, label="Yêu thích/tích hợp")
                    ],
                    delete_after=20
                )
                return
        ...
        try:
            ...
            try:
                ...
                await message.channel.send(
                    f"{message.author.mention} Bạn phải đợi đơn đặt hàng âm nhạc trước đây của bạn để thực hiện...",
                )
                    ...
                    view = SelectInteraction(
                        ...
                        opts=[
                            disnake.SelectOption(label="Bài hát", emoji="🎵",
                                                 description="Chỉ tải nhạc của liên kết.", value="music"),
                            disnake.SelectOption(label="Playlist", emoji="🎶",
    ...
    async def parse_song_request(self, message, text_channel, data, *, response=None, attachment: disnake.Attachment=None):
        ...
        if not message.author.voice:
            raise GenericError("Bạn phải nhập kênh giọng nói để yêu cầu một bài hát.")
        ...
        try:
            if message.guild.me.voice.channel != message.author.voice.channel:
                raise GenericError(
                    f"Bạn phải tham gia kênh <#{message.guild.me.voice.channel.id}> để yêu cầu một bài hát.")
        except AttributeError:
            pass
    ...
    async def node_connection_closed(self, node: wavelink.Node):
                    ...
                    try:
                        ...
                        try:
                            await player.text_channel.send(
                                "Trình phát đã bị chấm dứt do thiếu máy chủ âm nhạc...",
                                delete_after=11)
                        except:
                            pass
                        await player.destroy()
        ...
        while True:
            ...
            if retries == 30:
                print(f"{self.bot.user} - [{node.identifier}] Tất cả các nỗ lực để kết nối lại không thành công...")
                return
            ...
            backoff *= 1.5
            print(
                f'{self.bot.user} - Không thể kết nối lại với máy chủ [{node.identifier}] thử lại trong {int(backoff)}'
                 f' giây. Lỗi: {error}')
            await asyncio.sleep(backoff)
            retries += 1
            continue
    ...
    async def wavelink_track_error(self, node, payload: wavelink.TrackException):
        ...
        print(("-" * 50) + f"\nLỗi khi chơi bài hát: {track.uri or track.search_uri}\n"
              f"Máy chủ: {player.node.identifier}\n"
              f"{error_format}\n" + ("-" * 50))
        ...
        if payload.error == "This IP address has been blocked by YouTube (429)":
            ...
            if newnode:
                ...
                embed = disnake.Embed(
                    color=self.bot.get_color(player.guild.me),
                    description="**Trình phát đã bị chấm dứt do thiếu máy chủ khả dụng.**"
                )
        ...
        if player.last_track:
            ...
            # TODO: Desativar esse recurso após a correção do lavaplayer ser efetuada.
                ...
                txt = "Máy chủ âm nhạc đã được khởi động lại để khắc phục sự cố và âm nhạc sẽ tiếp tục sau một vài " \
                       " giây (Vui lòng đợi)..."
    ...
    async def node_ready(self, node: wavelink.Node):
        msg = f'{self.bot.user} - Máy chủ âm nhạc: [{node.identifier}] đã sẵn sàng để sử dụng!'
        ...
        if max_retries:
            ...
            while not self.bot.is_closed():
                if retries >= max_retries:
                    print(
                        f"{self.bot.user} - Tất cả các nỗ lực kết nối với máy chủ [{data['identifier']}] thất bại.")
                    return
                else:
                    try:
    ...
    async def thread_song_request(self, thread: disnake.Thread):
        ...
        if self.bot.intents.message_content:
            embed.description = "**Bạn chỉ cần gửi tên/link bài hát vào là tui sẽ tự động thêm cho bạn" \
