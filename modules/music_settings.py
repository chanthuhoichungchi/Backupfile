...
class MusicSettings(commands.Cog):
    ...
    async def player_settings(self, inter: disnake.AppCmdInter):
        ...
        view.message = await func(
            embed=disnake.Embed(
                description="**Điều chỉnh cài đặt người chơi tiêu chuẩn:**",
    ...
    @commands.slash_command(
        description=f"{desc_prefix}Tạo/chọn một kênh chuyên dụng để đặt hàng nhạc và tạo một trình phát cố định.",
    ...
    async def setup(
            ...
            target: Union[disnake.TextChannel, disnake.VoiceChannel, disnake.ForumChannel, disnake.StageChannel] = commands.Param(
                name="channel", default=None, description="Chọn một kênh hiện có"
            ),
            purge_messages: str = commands.Param(
                name="delete_messages", default="no",
        ...
        if not target:
            ...
            buttons = [
                ...
                disnake.ui.Button(label="Hủy", custom_id=f"voice_channel_cancel_{id_}", emoji="❌")
            ]
            ...
            msg_select = await func(
                embeds=[
                    ...
                    ).set_footer(text="Bạn chỉ có 45 giây để nhấp vào nút."),
                    disnake.Embed(color=self.bot.get_color(guild.me)).set_footer(
                        text="Lưu ý: Nếu bạn muốn sử dụng một kênh diễn đàn, bạn sẽ phải chọn một kênh từ danh sách các kênh "
                              "bên dưới (Nếu bạn không có nó, bạn sẽ phải tạo kênh diễn đàn theo cách thủ công và sử dụng "
                              "ra lệnh lần nữa."
                    )
                ],
                components=[
        ...
        if isinstance(target, disnake.ForumChannel):
            ...
            if existing_channel and not guild.me.guild_permissions.administrator and not target.permissions_for(guild.me).manage_permissions:
                raise GenericError(f"**{guild.me.mention} không có quyền của quản trị viên hoặc "
                                    ...
                                    f" kênh {target.mention} để cho phép tôi quản lý quyền hoặc sử dụng lại lệnh "
        ...
        embed = disnake.Embed(
            description=f"**{msg}**\n\nLưu ý: Nếu bạn muốn loại bỏ cấu hình này, chỉ cần sử dụng lệnh {reset_txt} hoặc "
    ...
    async def add_dj_role(
        ...
        await inter.send(f"Role {role.mention}  đã được thêm vào danh sách DJ's.", ephemeral=True)
    ...
    async def remove_dj_role(
        ...
        if not guild_data['djroles']:
            ...
            await inter.send("Không có vị trí nào trong danh sách của DJ.", ephemeral=True)
            return
    ...
    @commands.has_guild_permissions(manage_guild=True)
    @commands.command(description="Thay đổi ngoại hình/da của người chơi.", name="changeskin", aliases=["skin"],
    ...
    async def change_skin(self, inter: disnake.AppCmdInter):
        ...
        for b in self.bot.pool.bots:
            ...
            player.setup_hints()
            player.process_hint()
            player.set_command_log(text=f"{inter.author.mention} Đã thay đổi giao diện của người chơi.", emoji="🎨")
            await player.invoke_np(force=True)
    ...
    async def listenalong(self, ctx: CustomContext):
        ...
        await ctx.reply(
            embed=disnake.Embed(
                description=f"**Tạo lời mời trong kênh {ctx.author.voice.channel.mention} bằng cách chọn "
                             f"\"Đăng ký với tư cách khách\" rồi nhấp vào nút bên dưới để gửi "
                             f"lời mời.**\n\n"
                             f"Thận trọng! Nếu bạn không có tùy chọn này, điều đó có nghĩa là tài nguyên không khả dụng trên "
                             f" và tôi khuyên bạn không nên tiếp tục để tránh cấp quyền truy cập vĩnh viễn cho thành viên sử dụng "
                             f" hoặc tránh các vấn đề về quyền, v.v."
            ).set_image(url="https://cdn.discordapp.com/attachments/554468640942981147/1108943648508366868/image.png").
            set_footer(text="Lưu ý: tạo lời mời không giới hạn như: hạn sử dụng, số lần sử dụng hay "
                             "chỉ dành cho một người dùng sử dụng."),
    ...
    async def send_listen_along_invite(self, inter: disnake.MessageInteraction):
        ...
        if not inter.author.voice.channel:
            return await inter.send("**Bạn cần phải ở trên kênh thoại để gửi lời mời.**", ephemeral=True)
    ...
    async def listen_along_modal(self, inter: disnake.ModalInteraction):
        ...
        if invite.target_user:
            return await inter.edit_original_message("Lời mời không thể được cấu hình cho chỉ 1 người dùng sử dụng.")
        ...
        await inter.edit_original_message(
            f"**liên kết {invite} đã được kích hoạt/cập nhật thành công để được gửi qua RPC khi có"
            f"Trình phát hoạt động trên kênh {inter.author.voice.channel.mention}.**\n"
            f"`Lưu ý: Nếu bạn muốn hiển thị trạng thái của mình và không có ứng dụng RPC, hãy sử dụng lệnh /rich_presence để "
             f" để biết thêm thông tin.`"
        )
    ...
    async def rpc_create_modal(self, inter: disnake.ModalInteraction):
        ...
        if inter.text_values["token_input"] == data["token"]:
            await inter.send("Mã thông báo của bạn bằng với mã thông báo hiện tại!", ephemeral=True)
            return
    ...
    async def nodeinfo(self, inter: disnake.AppCmdInter):
        ...
        for identifier, node in bot.music.nodes.items():
            ...
            status = "<:emoji_6150:1146254135331651695>" if current_player else "<:upvote:1146329733102047242>"
...
class RPCCog(commands.Cog):
    ...
    async def rich_presence(self, inter: disnake.AppCmdInter):
        ...
        embed = disnake.Embed(
            color=self.bot.get_color(),
            description="**Hướng dẫn nhỏ sử dụng ứng dụng để hiển thị bài hát bạn đang nghe qua RPC:\n\n"
                        "Tải xuống ứng dụng (musicbot_rpc.zip) "
                        "[đây](https://github.com/zRitsu/Discord-MusicBot-RPC/releases).\n\n"
                        "Trích xuất musicbot_rpc.zip và trong thư mục mở musicbot_rpc." \
                        "Thêm liên kết WebSocket bên dưới vào ứng dụng (Tab: Cài đặt ổ cắm):** ```ansi\n" \
            ...
            components.extend(
                [
                    ...
                    disnake.ui.Button(label="Nhập/Chỉnh sửa/Xem mã thông báo", custom_id=f"rpc_create.{inter.author.id}",
                                      emoji="✍️", row=0),
                    disnake.ui.Button(label="Xóa mã thông báo (Tắt)", custom_id=f"rpc_remove.{inter.author.id}",
                                      emoji="♻️", row=1),
                ]
            )

        embed.description += "\n\n**Bây giờ chỉ cần nhấp vào nút \"Bắt đầu công ty\" evà nghe nhạc qua" \
    ...
    async def rpc_button_event(self, inter: disnake.MessageInteraction):
        ...
        if button_id == "rpc_gen":
            ...
            msg = f"Mã thông báo để sử dụng trên ứng dụng RPC (sự hiện diện phong phú) đã được tạo thành công!\n\n" \
                  f"`Gelad quận:` ||{data['token']}||"
            ...
            await inter.response.send_modal(
                ...
                components=[
                    disnake.ui.TextInput(
                        style=disnake.TextInputStyle.short,
                        label="Keo mã thông báo trong trường dưới đây:",
                        placeholder="Lưu ý: Để đo lường an toàn, không bao giờ bao gồm mật khẩu cá nhân ở đây!",
