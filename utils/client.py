...
class BotPool:
    ...
    async def start_bot(self, bot: BotCore):
        ...
        try:
            ...
            if error.status == 429 or "429 Too Many Requests" in str(e):
                ...
                print(
                    "Ứng dụng với Ratelimit từ Discord!\n"
        ...
        if e:
            ...
            if isinstance(e, disnake.PrivilegedIntentsRequired):
                e = "Bạn chưa bật Ý định đặc quyền trong ứng dụng của mình<br>" \
                     "Truy cập cổng nhà phát triển discord:<br>" \
                     "https://discord.com/developers/applications/<br>" \
                     "và nhấp vào ứng dụng của bạn rồi nhấp vào menu \"bot\"<br>" \
                     "và sau đó kích hoạt tất cả ý định.<br>" \
                     "Bản in ví dụ: https://i.imgur.com/a9c1DHT.png<br>" \
                     "Sau khi sửa xong khởi động lại ứng dụng."

                print(("=" * 30) + f"\nKhông khởi động bot được cấu hình trên: {bot.identifier}\n" + e.replace('<br>', '\n') + "\n" + ("=" * 30))

            elif isinstance(e, disnake.LoginFailure) and "Improper token" in str(e):
                e = "Mã thông báo không hợp lệ đã được sử dụng.<br>" \
                     "Kiểm tra xem mã thông báo đã nhập có chính xác không<br>" \
                     "hoặc nếu mã thông báo đã được đặt lại<br>" \
                     "hoặc được sao chép từ đúng vị trí (ví dụ: https://i.imgur.com/k894c1q.png )<br>" \
                     "Sửa xong khởi động lại ứng dụng."
    ...
    def setup(self):
        ...
        for key, value in self.config.items():
            ...
            if key.lower().startswith("lavalink_node_"):
                try:
                    ...
                    print(f"Thất bại khi thêm nút: {key}, erro: {repr(e)}")
        ...
        if mongo_key:
            ...
            print(Fore.YELLOW + f"Mã thông báo/liên kết của MongoDB chưa được cấu hình...\nCác tệp cơ sở dữ liệu sẽ được lưu cục bộ "
                  f"trong thư mục: local_database\n{'-' * 30}")

        self.local_database = LocalDatabase()

        try:
            self.commit = check_output(['git', 'rev-parse', 'HEAD']).decode('ascii').strip()
            print(f"Phiên bản hiện tại là: {self.commit}\n{'-' * 30}")
        except:
            self.commit = None
        ...
        def load_bot(bot_name: str, token: str):
            ...
            if not token:
                print(f"{bot_name} Bị bỏ qua (Token không được cung cấp)...")
                return
            ...
            async def on_ready():
                        ...
                        if str(bot.user.id) in bot.config["INTERACTION_BOTS"] or \
                            ...
                            if self.config["INTERACTION_BOTS"] and self.config["ADD_REGISTER_COMMAND"]:
                                ...
                                async def register_commands(
                                    ...
                                    embed = disnake.Embed(
                                        description="**Sự chú ý! ** Tất cả các lệnh thanh của tôi (/) hoạt động thông qua ứng dụng "
                                                    f"với một trong những cái tên bên dưới:**\n{interaction_invites}\n\n"
                                                    "**Nếu các lệnh ứng dụng ở trên không được hiển thị khi vào thanh (/), "
                                    ...
                                    if not inter.author.guild_permissions.manage_guild:
                                        embed.description += "\n\n**Lưu ý: ** Sẽ cần phải có quyền của ** Quản lý" \
                ...
                print(Fore.CYAN + f'{bot.user} - [{bot.user.id}] Online.')
        ...
        if not self.bots:
            ...
            message = "Mã thông báo bot chưa được cấu hình đúng cách!\n\n"

            if os.environ.get("REPL_SLUG"):
                message += f"Kiểm tra xem mã thông báo có được thêm vào bí mật sao chép không"

                print(message + ": Làm thế nào để cấu hình: https://gist.github.com/zRitsu/70737984cbe163f890dae05a80a3ddbe#2---com-o-projeto-j%C3%A1-criado-prossiga-as-etapas-abaixo")
                ...
                message += "Kiểm tra xem mã thông báo có được cấu hình trong không ENV/ENVIRONMENT hoặc trong tệp .env"
...
class BotCore(commands.Bot):
    ...
    def load_skins(self):
        ...
        for skin in os.listdir("./utils/music/skins/normal_player"):
            ...
            try:
                ...
                if not hasattr(skin_file, "load"):
                    print(f"Skin làm ngơ: {skin} | Tải () hàm không được cấu hình/tìm thấy...")
                    continue
                self.player_skins[skin[:-3]] = skin_file.load()
            except Exception:
                print(f"Không mang theo skin [normal_player]: {traceback.format_exc()}")
        ...
        for skin in os.listdir("./utils/music/skins/static_player"):
            ...
            try:
                ...
                print(f"Không mang theo skin [static_player]: {traceback.format_exc()}")
    ...
    async def can_send_message(self, message: disnake.Message):
        ...
        if not message.channel.permissions_for(message.guild.me).send_messages:
            ...
            print(f"Không thể gửi tin nhắn vào: {message.channel.name} [{message.channel.id}] (Thiếu quyền)")
    ...
    async def on_message(self, message: disnake.Message):
        ...
        if isinstance(message.channel, disnake.StageChannel):
            ...
            embed.description = f"**Xin chào {message.author.mention} nha.\n\n" \
                                f"Để xem tất cả các lệnh của tui hãy sử dụng: /**"
            ...
            if not self.command_sync_flags.sync_commands and self.config["INTERACTION_BOTS"]:
                ...
                if interaction_invites:
                    embed.description += f"\n\nCác lệnh gạch chéo lên (/) của tôi hoạt động thông qua " \
                                          f"trong số các ứng dụng sau bên dưới:\n" \
                                          f"{' **|** '.join(interaction_invites)}\n\n" \
                                          f"Nếu các lệnh ứng dụng trên không hiển thị khi gõ " \
                                          f" dấu gạch chéo (/), hãy nhấp vào tên bên trên để nhúng các lệnh dấu gạch chéo vào " \
                                          f"máy chủ của bạn."

            if not self.config["INTERACTION_COMMAND_ONLY"]:
                embed.description += f"\n\nTôi cũng có các lệnh văn bản bằng tiền tố.\n" \
            ...
            if isinstance(channel.parent, disnake.ForumChannel):
                ...
                if channel.owner_id in (bot.user.id for bot in self.pool.bots if bot.bot_ready):
                    ...
                    raise GenericError("**Bạn không thể sử dụng các lệnh có tiền tố trong bài đăng hiện tại...**\n"
                                       "`Sử dụng lệnh thanh (/) đây.`", self_delete=True)
        except AttributeError:
            pass
    ...
    async def on_application_command(self, inter: disnake.ApplicationCommandInteraction):
        ...
        if not self.bot_ready:
            await inter.send("Tôi vẫn đang khởi động...\nVui lòng đợi lâu hơn một chút...", ephemeral=True)
            return

        if self.config["COMMAND_LOG"] and inter.guild:
            try:
                print(Fore.BLUE + f"Nhật ký CMD: [người dùng: {inter.author} - {inter.author.id}] - [máy chủ: {inter.guild.name} - {inter.guild.id}]"
