...
def can_send_message(
    ...
    if not perms.embed_links:
        raise GenericError(f"**{bot.mention} Không có quyền chèn các liên kết vào kênh: {channel.mention}**")
...
async def check_requester_channel(ctx: CustomContext):
    ...
    if guild_data['player_controller']["channel"] == str(ctx.channel.id):
        ...
        raise GenericError("**Chỉ sử dụng lệnh thanh (/) trên kênh này!**", self_delete=True, delete_original=15)
...
async def check_pool_bots(inter, only_voiced: bool = False, check_player: bool = True, return_first=False):
    ...
    if not inter.guild:
        ...
        msg = "**Tất cả các bot đang được sử dụng...**"
        if extra_bots_counter:
            components = [disnake.ui.Button(custom_id="bot_invite", label="Cần thêm các bot âm nhạc? Bấm vào đây.")]
...
def can_send_message_check():
    ...
    async def predicate(inter):
        ...
        if not inter.guild:
            ...
            raise GenericError("**Lệnh này nên được sử dụng trên máy chủ...**")
...
def check_queue_loading():
    ...
    async def predicate(inter):
        ...
        if player.locked:
            raise GenericError("**Không thể thực hiện hành động này trong khi quá trình xử lý nhạc đang diễn ra"
