...
def parse_error(
    ...
    if isinstance(error, NotDJorStaff):
        ...
        txt = {
            ...
            commands.BucketType.channel: f"Bạn đã sử dụng lệnh này rồi {txt}Trên kênh hiện tại",
            commands.BucketType.category: f"Bạn đã sử dụng lệnh này rồi {txt}Trong danh mục kênh hiện tại",
            commands.BucketType.role: f"Bạn đã sử dụng lệnh này rồi {txt}bởi một thành viên có vị trí được phép",
            commands.BucketType.default: f"Bạn đã sử dụng lệnh này rồi {txt}cho một ai đó"
        }

        error_txt = f"{ctx.author.mention} **{txt[error.per]} và việc sử dụng nó vẫn chưa được hoàn thiện!" 
