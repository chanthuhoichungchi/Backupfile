...
class VolumeInteraction(disnake.ui.View):
    ...
    def process_buttons(self):
        ...
        select = disnake.ui.Select(placeholder='Chọn:', options=opts)
...
class QueueInteraction(disnake.ui.View):
    ...
    def update_pages(self):
        ...
        for n, page in enumerate(self.pages):
            ...
            for t in page:
                ...
                txt += f"`┌ {counter})` [`{fix_characters(t.title, limit=50)}`]({t.uri})\n" \
                       f"`└ ⏲️ {duration}`" + (f" - `Sự lặp lại: {t.track_loops}`" if t.track_loops else  "") + \
                       f" **|** `✋` <@{t.requester}>\n"
        ...
        track_select = disnake.ui.Select(
            placeholder="TMột bài hát cụ thể trên trang:",
    ...
    async def on_timeout(self) -> None:
        ...
        embed.set_footer(text="Thời gian để tương tác đã hết!")
    ...
    def update_embed(self):
        self.embed.title = f"**bài hát xếp hàng [{self.current+1} / {self.max_page+1}]**"
