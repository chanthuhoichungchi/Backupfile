...
def download_file(url, filename):
    if os.path.isfile(filename):
        return
    print(f"Tải xuống tập tin: {filename}")
...
def validate_java(cmd: str, debug: bool = False):
    try:
        ...
        if debug:
            print(f"\nKhông nhận được phiên bản Java...\n"
...
def run_lavalink(
    ...
    print(f"Khởi động máy chủ Lavalink (tùy thuộc vào việc lưu trữ Lavalink có thể bắt đầu, "
          f"Điều gì có thể xảy ra thất bại trong một số lần thử kết nối cho đến khi nó bắt đầu đầy đủ).\n{'-' * 30}")
