...
class LavalinkPlayer(wavelink.Player):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.bot.config["USE_YTDL"] or self.bot.spotify:
            self.initial_hints.append(
                ...
                f"Thử sử dụng lệnh /tích hợp hoặc {self.prefix_info}integrations."
            )
    ...
    def __str__(self) -> str:
        return f"Node đang sử dụng: {self.node.identifier}"
    ...
    def setup_hints(self):
        ...
        if self.static:
            hints.append("Có thể ghim các bài hát/danh sách phát trong tin nhắn trình phát khi ở "
                          "chờ/không hoạt động để cho phép các thành viên nghe công khai. Để thực hiện việc này, hãy sử dụng "
                          f"command /server_playlist hoặc {self.prefix_info}serverplaylist (chỉ thành viên có quyền quản lý "
                          "máy chủ có thể sử dụng lệnh này).")

        elif self.bot.intents.message_content and self.controller_mode:
            hints.append("Khi tạo hội thoại/chủ đề trong tin nhắn của người chơi, chế độ yêu cầu bài hát sẽ được kích hoạt "
                          "trong đó (có thể yêu cầu nhạc chỉ bằng cách gửi tên/liên kết của bài hát trong cuộc trò chuyện).")

        if len([b for b in self.bot.pool.bots if b.appinfo and b.appinfo.bot_public]) > 1:
            hints.append("Có thể có các chương trình âm nhạc bổ sung trên máy chủ chia sẻ tất cả các mục yêu thích của chúng không/"
                          "tích hợp và làm việc với một tiền tố duy nhất và lệnh gạch chéo từ một bot duy nhất."
                          f"Bạn có thể sử dụng lệnh /invite hoặc {self.prefix_info}invite để thêm họ.")

        if self.controller_mode:
            hints.append(
                "Bằng cách nhấp vào biểu tượng cảm xúc này 🎛️ từ thông báo của một số lệnh, bạn sẽ được chuyển hướng đến bộ điều khiển trình phát."
            )
    ...
    async def members_timeout(self, check: bool, force: bool = False):
        ...
        if check:
            ...
            if update_log:
                self.set_command_log(emoji="🔰", text="Âm nhạc tiếp tục từ tự động tạm dừng.")
                if self.current:
        ...
        if self.keep_connected:
            ...
            self.set_command_log(text=f"Trình phát đã bị tạm dừng do thiếu thành viên trên kênh. MỘT "
                                      f"Âm nhạc sẽ tự động tiếp tục khi một thành viên tham gia kênh "
        ...
        if isinstance(track, PartialTrack):
            ...
            if not track.id:
                try:
                    await self.text_channel.send(
                        embed=disnake.Embed(
                            description=f"Bài hát [{track.title}]({track.uri}) không khả dụng...\n"
    ...
    async def process_idle_message(self):
        ...
        if not self.static and not self.controller_mode:
            ...
            try:
                ...
                embed = disnake.Embed(
                    description=f"**Đã hết bài hát... Sử dụng một trong các lệnh bên dưới để thêm bài hát hoặc dừng "
                                 f"người chơi.**\n\n`{cmds}`\n\n"
                                f"**Lưu ý:** `Trình phát sẽ tự động tắt sau` "
                                f"<t:{int((disnake.utils.utcnow() + datetime.timedelta(seconds=self.idle_timeout)).timestamp())}:R> "
                                f"`Nếu không có lệnh nào được sử dụng...`",
            ...
            controller_opts.extend(
                [
                    disnake.SelectOption(
                        ...
                        description=f"Nghe lại: {self.played[-1].title[:31]}"
                    ),
                    disnake.SelectOption(
                        label="Bật tự động phát", emoji="🔄",
                        value=PlayerControls.autoplay,
                        description=f"Phát nhạc liên quan đến: {self.played[-1].title[:19]}"
                    ),
                ]
            )
        ...
        if played > 1:
            controller_opts.append(
                disnake.SelectOption(
                    emoji="↪️", value=PlayerControls.readd, label="chơi lại",
        ...
        controller_opts.append(
            disnake.SelectOption(
                emoji="🛑", value=PlayerControls.stop, label="Tắt",
    ...
    async def idling_mode(self):
        ...
        msg = "💤 **⠂Người chơi đã bị tắt bởi sự không hoạt động...**"
