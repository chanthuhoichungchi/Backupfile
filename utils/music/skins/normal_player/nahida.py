...
class DefaultProgressbarSkin:
    ...
    def load(self, player: LavalinkPlayer) -> dict:
        ...
        txt = f"[`{player.current.single_title}`]({player.current.uri})\n\n" \
              f"{duration1}\n" \
              f"> <:KusaDerp:1131541983853285376>  **⠂Tác giả** {player.current.authors_md}\n" \
              f"> <:Deep_Thought:1128552268036329492> **⠂Người gọi bài:** <@{player.current.requester}>\n" \
              f"> <:NahidaVibe:1137343239511351386> **⠂Âm lượng:** `{player.volume}%`\n " \

        if player.current.track_loops:
            txt += f"\n> <:KusaLoading:1131579491609747558> **⠂Lặp lại còn lại:** `{player.current.track_loops}`\n " \
        ...
        if player.loop:
            if player.loop == 'current':
                e = '<:KusaLoading:1131579491609747558>'
                m = 'Bài hát hiện tại'
            else:
                e = '<:KusaLoading:1131579491609747558>'
                m = 'Hàng'
        ...
        if player.keep_connected:
            txt += f"\n> <:KusaSleep:1132377031741546506> **⠂Chế độ 24/7:** `Kích hoạt`"
        ...
        if qlenght and player.mini_queue_enabled:
            ...
            embed_queue.set_image(url="https://media.discordapp.net/attachments/1133406037056159905/1138433186808995851/nahida_dance.gif")

        embed.description = txt
        embed.set_image(url="https://media.discordapp.net/attachments/1133406037056159905/1138433186808995851/nahida_dance.gif")
        embed.set_thumbnail(url=player.current.thumb)

        data["embeds"] = [embed_queue, embed] if embed_queue else [embed]

        data["components"] = [
            disnake.ui.Button(emoji="<:Giving_Comments:1128535761206124644> ", custom_id=PlayerControls.stop, style=disnake.ButtonStyle.red),
            ...
            disnake.ui.Button(emoji="<:Lunch_Break:1128545379781193768>", custom_id=PlayerControls.add_song, style=disnake.ButtonStyle.green, label="Thêm nhạc"),
            disnake.ui.Select(
