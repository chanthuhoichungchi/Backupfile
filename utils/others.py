    ...
    async def interaction_check(self, interaction: disnake.MessageInteraction) -> bool:
        ...
        if interaction.author != self.ctx.author:
            await interaction.send(
                f"Chỉ có thành viên {self.ctx.author.mention} mới có thể sử dụng các nút của tin nhắn này...",
                ephemeral=True
            )
            return False
...
def sync_message(bot: BotCore):
    ...
    return f"`Nếu các lệnh gạch chéo không xuất hiện, `[`nhấp vào đây`]({app_commands_invite}) `để cho phép tôi " \
            "tạo lệnh gạch chéo trên máy chủ.`\n\n" \
            "`Lưu ý: Trong một số trường hợp, các lệnh gạch chéo có thể mất tới một giờ để xuất hiện/cập nhật trong tất cả " \
            "các máy chủ. Nếu bạn muốn sử dụng các lệnh gạch chéo ngay lập tức trên máy chủ, bạn sẽ phải " \
            f"đá tôi ra khỏi máy chủ và sau đó thêm lại tôi qua `[`link`] này({bot_invite})..."
...
async def send_idle_embed(
    ...
    embed = disnake.Embed(description="**Nhập một kênh thoại và yêu cầu một bài hát tại đây " +
                                       ("trong bài đăng" if is_forum else "trong kênh hoặc cuộc trò chuyện bên dưới") +
                                       f" (hoặc nhấp vào nút bên dưới hoặc sử dụng lệnh {cmd} tại đây hoặc trong một số kênh khác)**\n\n"
    ...
    if text:
        embed.description += f"**Hành động cuối cùng:** {text.replace('**', '')}\n"
...
async def select_bot_pool(inter, first=False):
    ...
    if len(bots) == 1 or first:
        ...
        try:
            ...
            try:
                await msg.edit(conent="Hết thời gian chọn!", embed=None, view=None)
            except:
                pass
            return None, None
        ...
        try:
            ...
            raise GenericError("**Bot đã chọn đã bị xóa khỏi máy chủ trước khi bạn lựa chọn..**")
