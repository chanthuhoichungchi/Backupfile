...
class IndexHandler(tornado.web.RequestHandler):
    ...
    async def get(self):
        ...
        msg += f"<p><a href=\"https://github.com/zRitsu/DC-MusicBot-RPC" \
              f"/releases\" target=\"_blank\">Tải xuống ứng dụng hiện diện phong phú tại đây.</a></p>Liên kết để thêm vào ứng dụng " \
              f"de RPC: {ws_url}"

        if self.config["ENABLE_RPC_AUTH"]:
            msg += f"\nĐừng quên lấy mã thông báo để định cấu hình trong ứng dụng, sử dụng lệnh /rich_presence để có được một.\n<br><br>"

        if self.pool.commit:
            msg += f"\nPhiên bản hiện tại: <a href=\"{self.pool.remote_git_url}/commit/{self.pool.commit}\" target=\"_blank\">{self.pool.commit[:7]}</a>"
...
class WebSocketHandler(tornado.websocket.WebSocketHandler):
    ...
    def on_message(self, message):
        ...
        if not ws_id:
            ...
            if not bot_id:
                print(f"ngắt kết nối: vì thiếu id người dùng {self.request.remote_ip}\nDados: {data}")
                self.write_message(json.dumps({"op": "disconnect", "reason": "Ngắt kết nối do thiếu id người dùng"}))
            ...
            try:
                ...
                print(f"Lỗi xử lý dữ liệu từ rpc tới người dùng [{data['user']}]: {repr(e)}")
        ...
        print("\n".join(f"kết nối mới - Người dùng: {u}" for u in self.user_ids))
